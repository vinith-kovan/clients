...
export class StateService extends BaseStateService<GlobalState, Account> {
  ...
  async getBannerStates(options?: StorageOptions): Promise<{ [id: string]: boolean }> {
    options = this.reconcileOptions(options, await this.defaultOnDiskOptions());
    const account = await super.getAccount(options);
    return account.settings.bannerStates;
  }

  async setBannerStates(value: { [id: string]: boolean }, options?: StorageOptions): Promise<void> {
    options = this.reconcileOptions(options, await this.defaultOnDiskOptions());
    const account = await super.getAccount(options);
    account.settings.bannerStates = value;
    await super.saveAccount(account, options);
  }
